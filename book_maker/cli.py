...
def parse_prompt_arg(prompt_arg):
    ...
    if (prompt.keys() - {"user", "system"}) != set():
        raise ValueError(
            "prompt can only contain the keys of `user` and `system`")
...
""",
    ...
    if options.deployment_id:
        ...
        if not options.api_base:
            raise ValueError(
                "`api_base` must be provided when using `deployment_id`")
...
""",
    ...
    if options.book_from == "kobo":
        ...
        input_options.book_name = obok.cli_main(device_path)

    book_type = input_options.book_name.split(".")[-1]
    support_type_list = list(BOOK_LOADER_DICT.keys())
    if book_type not in support_type_list:
        raise Exception(
            f"now only support files of these formats: {','.join(support_type_list)}",
    ...
    book_loader = BOOK_LOADER_DICT.get(book_type)
    assert book_loader is not None, "unsupported loader"
    language = input_options.language
    if input_options.language in LANGUAGES:
        # use the value for prompt
        language = LANGUAGES.get(language, language)

    # change api_base for issue #42
    model_api_base = input_options.api_base

    e = book_loader(
        input_options.book_name,
        translate_model,
        API_KEY,
        options.resume,
        language=language,
    ...
    if options.retranslate:
        e.retranslate = options.retranslate
    if input_options.deployment_id:
        # only work for ChatGPT api for now
        # later maybe support others
        assert (
            options.model == "chatgptapi"
        ), "only support chatgptapi for deployment_id"
        if not input_options.api_base:
            raise ValueError(
                "`api_base` must be provided when using `deployment_id`")
        e.translate_model.set_deployment_id(input_options.deployment_id)
